...
describe("GOV", function () {
  ...
  it("Initial balanceOf testing", async function () {
	expect(await gov.connect(account_0).balanceOf(account_0.address)).to.equal("10000");
	expect(await gov.connect(account_1).balanceOf(account_1.address)).to.equal("0");
  });
  
});


describe("ASSET_A", function () {
